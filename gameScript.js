...
window.onload = function() {
    $("#ol").html(`<center><div id="inst"><h3>Welcome !</h3>Instructions For Game<br/><br/><li>Make pairs of similiar blocks by flipping them.</li><li>To flip a block you can click on it.</li><li>If two blocks you clicked are not similar, they will be flipped back.</li><p style="font-size:18px;">Click one of the following mode to start the game.</p></div><button onclick="startGame(3, 4)">3 x 4</button> <button onclick="startGame(4, 4)" style="w">4 x 4</button><button onclick="startGame(4, 5)">4 x 5</button><button onclick="startGame(5, 6)">5 x 6</button><button onclick="startGame(6, 6)">6 x 6</button></center>`);
}
...
function startGame(r,l) {
    ...
    for (var i = 1;i<=r;i++) {
        $("table").append("<tr>");
        for (var j = 1;j<=l;j++) {
           $("table").append(`<td id='${n}' onclick="flipCard(${n})"><div class='inner'><div class='front'></div><div class='back'><p>${items[n-1]}</p></div></div></td>`);
           n++;
         }
         $("table").append("</tr>");
    }
...
function flipCard(x) {
  ...
  //Flip
    ...
    if (rem==0) {
          ...
          setTimeout(function() {
              $("#ol").html(`<center><div id="iol"><h2>Congrats!</h2><p style="font-size:23px;padding:10px;">You completed the ${mode} mode in ${moves} moves. It took you ${time}.</p><p style="font-size:18px">Comment Your Score!<br/>Play Again ?</p><button onclick="startGame(3, 4)">3 x 4</button> <button onclick="startGame(4, 4)" style="w">4 x 4</button><button onclick="startGame(4, 5)">4 x 5</button><button onclick="startGame(5, 6)">5 x 6</button><button onclick="startGame(6, 6)">6 x 6</button></div></center>`);
              $("#ol").fadeIn(750);
          }, 1500);
    }
  }
